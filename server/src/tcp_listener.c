...
#define MAX_ALERTAS_EN_RESPUESTA 20

typedef struct {
    int socket_cliente;
} ArgOperador;

static void construir_respuesta_status(const char *id, char *respuesta, size_t tam) {
    ...
    if (nodo == NULL) {
        ...
        snprintf(respuesta, tam,
                 "STATUS|%s|TEMP|%.2f|HUM|%.2f|CONSUMO|%.2f|VIBRACION|%.2f|ESTADO|%s|CONEXION|%s\n",
                 nodo->id, nodo->temperatura, nodo->humedad,
                 nodo->consumo_energetico, nodo->vibracion, nodo->estado, actividad);
    }
...
static void construir_respuesta_alertas(char *respuesta, size_t tam) {
    ...
    // Se envían como máximo las últimas MAX_ALERTAS_EN_RESPUESTA, de más
    // reciente a más antigua, para no desbordar el buffer de respuesta.
    // El número en el encabezado SIEMPRE refleja cuántas se están enviando
    // realmente, nunca el total histórico (bug original: se mandaba
    // "ALERTS|200" pero solo cabían ~15 en el buffer de 512 bytes).
    int a_enviar = cantidad_alertas < MAX_ALERTAS_EN_RESPUESTA
                       ? cantidad_alertas
                       : MAX_ALERTAS_EN_RESPUESTA;

    int enviadas = 0;
    char cuerpo[BUFFER_SIZE] = "";

    for (int i = cantidad_alertas - 1; i >= 0 && enviadas < a_enviar; i--) {
        char item[64];
        snprintf(item, sizeof(item), "|%s:%s:%.2f:%ld",
                 historial_alertas[i].nodo_id,
                 historial_alertas[i].variable,
                 historial_alertas[i].valor,
                 (long)historial_alertas[i].timestamp);

        if (strlen(cuerpo) + strlen(item) >= sizeof(cuerpo) - 1) {
            break;
        }

        strncat(cuerpo, item, sizeof(cuerpo) - strlen(cuerpo) - 1);
        enviadas++;
    }

    snprintf(respuesta, tam, "ALERTS|%d%s\n", enviadas, cuerpo);
...
static void construir_respuesta_system_status(char *respuesta, size_t tam) {
    ...
    snprintf(respuesta, tam,
             "SYSTEM_STATUS|NODOS_TOTAL|%d|NODOS_ACTIVOS|%d|ALERTAS_TOTAL|%d|UPTIME_SEG|%ld\n",
...
void *hilo_tcp(void *arg) {
    ...
    while (!apagando) {
        ...
        int sock_cliente = accept(sock_tcp, (struct sockaddr *)&cliente, &cliente_len);
